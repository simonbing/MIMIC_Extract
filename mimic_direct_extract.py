...
def save_numerics(
    ...
    # First convert everything to seconds, then back to whatever interval we want
    to_hours = lambda x: max(0, (x.days*24*60*60 + x.seconds) // (60 * time_step))
...
if __name__ == '__main__':
    ...
    dynamic_hd5_filename = splitext(dynamic_hd5_filename)[0] + '_' + F"{args['time_step']}_mins" + '_' + pop_size + splitext(dynamic_hd5_filename)[1]
    outcome_filename = splitext(outcome_filename)[0] + '_' + F"{args['time_step']}_mins" + '_' + pop_size + splitext(outcome_filename)[1]
    dynamic_hd5_filt_filename = splitext(dynamic_hd5_filt_filename)[0] + '_' + F"{args['time_step']}_mins" + '_' + pop_size + splitext(dynamic_hd5_filt_filename)[1]
    outcome_hd5_filename = splitext(outcome_hd5_filename)[0] + '_' + F"{args['time_step']}_mins" + '_' + pop_size + splitext(outcome_hd5_filename)[1]
    ...
    idx_hd5_filename = splitext(idx_hd5_filename)[0] + '_' + F"{args['time_step']}_mins" + '_' + pop_size + splitext(idx_hd5_filename)[1]
